package
{
	...
	public class HelloHeatmap extends Sprite
	{
		public var map:Map;
		public var overlay:HeatMarkerClip;
		
		public function HelloHeatmap()
		{
			...
			for (var i:int = 0; i < 2048; i++) {
				var w:Number = 3.0*(Math.random()-0.5);
				var h:Number = 3.0*(Math.random()-0.5);
...
class HeatMarkerClip extends Sprite
{
	...
	protected var nativeZoom:int; 
	protected var nativeRadius:int; 
	protected var gradientArray:Array;

	// calculated from nativeZoom and nativeRadius
	protected var itemRadius:int;
	
    // TODO: calculate this from the number of points that overlap
    protected var centerValue:int = 128;
	...
	public function HeatMarkerClip(map:Map, locations:Array, gradientArray:Array, nativeRadius:int=25, nativeZoom:int=11)
	{
    	buttonMode = false;
    	mouseEnabled = false;
    	mouseChildren = true;
    ...
    protected function drawHeatMap(event:MapEvent=null):void
    {
    	if (!dirty) {
    		return;
    	}
        ...
        if(!heatBitmapData || heatBitmapData.width != w || heatBitmapData.height != h) {
        	if (heatBitmapData) {
        		heatBitmapData.dispose();
        	}
    ...
    protected function onMapResized(event:MapEvent):void
    {
        ...
        y = map.getHeight() / 2;
        dirty = true;
        drawHeatMap(); // force redraw because flash seems stingy about it
        // TODO: timer for the redraw in case it's too heavy
    }
